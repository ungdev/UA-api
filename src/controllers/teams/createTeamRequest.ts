...
export default [
  ...
  validateBody(
    Joi.object({
      userType: Joi.string()
        ...
        .required()
        .error(new Error("L'utilisateur doit être un joueur ou un coach")),
    }),
  ),
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      if (!team) return notFound(response, Error_.TeamNotFound);

      // Check if the team is not locked
      if (team.lockedAt) return forbidden(response, Error_.TeamLocked);

      const { user } = getRequestInfo(response);

      // Check if the user is already asking for a team
      if (user.askingTeamId) return forbidden(response, Error_.AlreadyAskedATeam);
      ...
      if (error.code === 'API_COACH_MAX_TEAM') return forbidden(response, Error_.TeamMaxCoachReached);
