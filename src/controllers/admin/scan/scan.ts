...
import { Permission, Error as Error_ } from '../../../types';
...
export default [
  ...
  validateBody(
    Joi.object({
      qrcode: Joi.string().required().error(new Error(Error_.NoQRCode)),
    }),
  ),
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        logger.error(error);
        return badRequest(response, Error_.InvalidQRCode);
      }

      // Retrieve the user from the QR Code
      const user = await fetchUser(userId);

      if (!user) return notFound(response, Error_.UserNotFound);

      // Check if the user has paid
      if (!user.hasPaid) return forbidden(response, Error_.NotPaid);

      // Check if the user has already validated
      if (user.scannedAt) return forbidden(response, Error_.UserAlreadyScanned);
